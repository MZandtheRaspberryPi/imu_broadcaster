...
sensor_broadcaster_args_t parse_args(const int &argc, char *argv[]) {
  if (argc != 4) {
    ...
    std::cerr << "  arg: "
              << "i2c_device_addres (ex: /dev/i2c-3)" << std::endl;
    std::cerr << "  arg: "
              << "i2c_chip_address (ex: 0x28)" << std::endl;
    std::cerr << "  arg: "
              << "websocket_port (ex: 9000)" << std::endl;
    return parsed_args;
  }
  ...
  unsigned long int i2c_chip_address =
      std::strtoul(i2c_chip_address_str.c_str(), nullptr, 0);
  unsigned long int websocket_port =
      std::strtoul(websocket_port_str.c_str(), nullptr, 0);

  sensor_broadcaster_args_t parsed_args{
      i2c_device_address, static_cast<uint8_t>(i2c_chip_address),
      static_cast<uint16_t>(websocket_port), 0};

  std::cout << "args parsed:" << std::endl;
  std::cout << "i2c_device_address: " << parsed_args.i2c_device_address
            << std::endl;
  std::ios::fmtflags f(std::cout.flags());
  std::cout << "i2c_chip_address: "
            << "0x" << std::hex
            << static_cast<int>(parsed_args.i2c_chip_address) << std::endl;
  std::cout.flags(f);
...
int main(int argc, char *argv[]) {
  ...
  // or 0x28)
  //                                   id, address
  Adafruit_BNO055 bno = Adafruit_BNO055(55, parsed_args.i2c_chip_address,
                                        parsed_args.i2c_device_address);
  setup_bno(bno);
  ...
  while (!EXIT_FLAG) {
    ...
    imu_msgs::Triad *linear_acceleration =
    ...
    imu_msgs::Triad *magnetometer_vector =
        imu_msg.mutable_magnetometer_vector();
